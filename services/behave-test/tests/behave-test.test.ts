...
describe("PreState + PostState Integration", () => {
  ...
  it("works together for deterministic testing workflow", async () => {
    // 1. Set up initial state with PreState
    await PreState(db, schema, {
      users: [{ id: 1, name: "Ed" }],
      posts: []
    });

    // 2. Verify initial state with PostState
    await expect(
      PostState(db, schema, {
        users: [{ id: 1, name: "Ed" }],
        posts: []
    ...
    // 4. Assert the final state with PostState
    await expect(
      PostState(db, schema, {
        users: [{ id: 1, name: "Ed" }],
  ...
  it("supports subset assertions in PostState after PreState setup", async () => {
  ...
  it("detects changes correctly", async () => {
    ...
    // PostState should detect the change
    await expect(
      PostState(db, schema, {
        posts: [{ id: 30, userId: 1, title: "Original title" }] // This should fail
      })
    ).rejects.toThrow(/PostState assertion failed:/);
