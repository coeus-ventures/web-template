...
/**
 * PostState - Assert that DB equals the expected JSON state
 *
 ...
 */
export async function PostState<TSchema extends Record<string, unknown>>(
  db: LibSQLDatabase<TSchema>,
  schema: TSchema,
  expected: StateObject,
  opts?: PostStateOptions
): Promise<true> {
  const options = {
  ...
  if (hasErrors) {
    throw new Error("PostState assertion failed:" + errorLines.join("\n"));
  }
...
/**
 * PostStateFromFile - Load JSON state from file and assert with PostState
 *
 * @param db - Drizzle DB instance
 * @param schema - Drizzle schema object
 ...
 * @param opts - Configuration options
 */
export async function PostStateFromFile<TSchema extends Record<string, unknown>>(
  db: LibSQLDatabase<TSchema>,
  schema: TSchema,
  filePath: string,
  opts?: PostStateOptions
  ...
  return PostState(db, schema, state, opts);
}