...
/**
 * PreState - Deterministically set database into a known state for testing
 *
 ...
 */
export async function PreState<TSchema extends Record<string, unknown>>(
  db: LibSQLDatabase<TSchema>,
  schema: TSchema,
  state: StateObject,
  opts?: PreStateOptions
): Promise<void> {
  const options = {
    wipe: true,
    resetSequences: true,
...
/**
 * PreStateFromFile - Load JSON state from file and apply with PreState
 *
 * @param db - Drizzle DB instance
 * @param schema - Drizzle schema object
 ...
 * @param opts - Configuration options
 */
export async function PreStateFromFile<TSchema extends Record<string, unknown>>(
  db: LibSQLDatabase<TSchema>,
  schema: TSchema,
  filePath: string,
  opts?: PreStateOptions
  ...
  return PreState(db, schema, state, opts);
}